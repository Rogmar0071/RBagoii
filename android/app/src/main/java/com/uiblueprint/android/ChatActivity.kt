...
class ChatActivity : AppCompatActivity(), ChatMessageAdapter.MessageActionListener {
    ...
    private fun uploadFile(uri: Uri) {
        ...
        executor.execute {
            try {
                ...
                // Generate a unique ID before posting to the UI thread so the
                // polling lambda can reference it without a race condition.
                val inFlightId = "in_flight_${java.util.UUID.randomUUID()}"
                ...
                runOnUiThread {
                    ...
                    addInFlightFile(inFlightId, filename, IngestStatus.UPLOADING)
                }
                ...
                if (jobId != null) {
                    ...
                    pollIngestJob(jobId, apiKey, baseUrl) { json ->
                        ...
                        updateInFlightFile(inFlightId, IngestStatus.fromBackendStatus(status))
     ...
     * [inFlightId] must be a UUID-based string generated by the caller before
     * posting to the UI thread (ensures the polling lambda can reference it).
     * Must be called on the main thread.
     */
    private fun addInFlightFile(inFlightId: String, filename: String, status: IngestStatus) {
        chatFiles.add(
            0,
            ChatFile(
                id = inFlightId,
    ...
    /**
     * Update the status pill of an in-flight placeholder entry by its unique ID.
     * Must be called on the main thread.
     */
    private fun updateInFlightFile(inFlightId: String, status: IngestStatus) {
        val idx = chatFiles.indexOfFirst { it.id == inFlightId }
        if (idx >= 0) {
     ...
     * Must be called on the main thread.
     */
    private fun removeInFlightFile(inFlightId: String) {
        if (chatFiles.removeAll { it.id == inFlightId }) {
            fileAdapter.submitList(chatFiles.toList())
            updateFileListUI()
        }
    }
