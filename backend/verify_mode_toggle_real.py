...
def test_mode_toggle_real_entry():
    ...
    payload_normal = {"message": message, "conversation_id": conv_id, "agent_mode": False}

    response_normal = client.post(
        "/api/chat", json=payload_normal, headers={"Authorization": f"Bearer {TOKEN}"}
    )
    ...
    payload_strict = {"message": message, "conversation_id": conv_id_strict, "agent_mode": True}

    response_strict = client.post(
        "/api/chat", json=payload_strict, headers={"Authorization": f"Bearer {TOKEN}"}
    )
