...
def execute_scenario_mode_engine(scenario: ExecutionScenario) -> ExecutionTrace:
    ...
    if MODE_STRICT in scenario.modes:
        trace.intent_object = IntentObject(
            domain="unknown", objective="unknown", expected_output_type="unknown"
        )
...
def assert_output_correctness(scenario: ExecutionScenario, trace: ExecutionTrace):
    ...
    if scenario.expect_structured_failure:
        ...
        assert trace.is_structured_failure, "Expected structured failure output"
        assert trace.failure_data is not None, "Structured failure should have data"
...
def detect_failure_surfaces(scenario: ExecutionScenario, trace: ExecutionTrace) -> list[str]:
    ...
    if (
        ...
        and trace.governance_status == "approved"
        and trace.validation_results
        and not all(vr.passed for vr in trace.validation_results)
    ):
    ...
    if (
        MODE_STRICT in trace.modes
        and trace.governance_status == "blocked"
        and trace.validation_results
        and all(vr.passed for vr in trace.validation_results)
    ):
    ...
    if (
        trace.validation_results
        and trace.contract_object is not None
        and MODE_STRICT in trace.modes
    ):
...
def verify_hard_invariants(scenario: ExecutionScenario, trace: ExecutionTrace):
    ...
    if len(trace.validation_results) > 0:
        ...
        assert MODE_STRICT in trace.modes, "INVARIANT VIOLATION: Validation without strict mode"
...
class TestExecutionPathVerification:
    ...
    def test_mode_isolation_preserved(self):
        ...
        assert len(normal_trace.validation_results) == 0, "Normal mode should have no validation"

        # Run strict mode
        strict_trace = execute_scenario_mode_engine(strict_scenario)
        assert len(strict_trace.validation_results) > 0, "Strict mode should have validation"
    ...
    def test_contract_binding_verified(self):
        ...
        assert len(trace.validation_results) > 0, "Strict mode should have validation"

        # Contract should be inferred/created
        assert MODE_STRICT in trace.modes, "Strict mode flag should be present"
